...
namespace qlCSYT
{
    public partial class frm_Nc_viewHSBA : Form
    {
        public frm_Nc_viewHSBA()
        {
            InitializeComponent();
            LoadHSBA();
        }
        private void LoadHSBA()
        {
            ...
            {
                conn.Open();
                OracleCommand cmd = new OracleCommand("Select * from CSYT_ADMIN.VIEW_NGHIENCUU_HSBA", conn);
        ...
        void DataGridViewRole_ShowServiceCellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == gv_HSBA.Columns["dataGridViewShowServiceButton"].Index)
            {
                ...
                OracleConnection conn = DBUtils.GetDBConnection();
                frm_Nc_viewHSBA_DV frm = new frm_Nc_viewHSBA_DV();
